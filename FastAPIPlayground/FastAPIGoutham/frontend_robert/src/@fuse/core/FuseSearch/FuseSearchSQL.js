// The FuseSearch, adapted to work with P-SQL
...
// Define the appearance of the input component
...
// Define the appearance of the suggestion component
...
// get the suggestion. Technical function. Likely do not have to change
...
// May need to adapt to search P-SQL database
function getSuggestionValue(suggestion) {
    return suggestion.title;
}

// Visual style again -- NOT critical function
...
// Set the initial state, no text, nothing displayed, etc
const initialState = {
    searchText: '',
    search: false,
    navigation: null,
...
function reducer(state, action) {
    switch (action.type) {
        ...
        case 'updateSuggestions': {
            // Get the value and link of the suggested item
...
// Function: getItemsFromSQL
// Edited from FuseUtils.getFlatNavigation()
// Return the items from the backend, so it can be displayed on the FuseSearch
// 1. Connect to the database
// 2. Return a list of items. Each item has: id, title (name), url. Leave type, icon and auth

async function getItemFromSQL(q) {
...
// const result = getItemFromSQL("bulb");


function FuseSearch(props) {
    const userRole = useSelector(({auth}) => auth.user.role);

    // Select the Navigations, and translate them
    // selectNavigation returns a list of results (items, in this case Navigations).
    ...
    useEffect(() => {
        ...
        // This step get the navigation (list of routes) and pass it to Search
        function setNavigation() {
            dispatch({
                type: 'setNavigation',
                // nav items are passed to getFlatNavigation, so it becomes a flat list
                // Then, itemAuthAllowed only gets the ones user have the auth to read
                // And returns the items into the list

                // value: FuseUtils.getFlatNavigation(navigation).filter(item => itemAuthAllowed(item))
                value: getItemFromSQL(state.searchText)
                // value: navigation
            });
            // console.log(FuseUtils.getFlatNavigation(navigation).filter(item => itemAuthAllowed(item)))
        }

        setNavigation();
    }, [userRole, navigation]);

    // show search (open the menu)
    ...
    // hide the search. Not relevant to search results
    function hideSearch() {
        dispatch({type: 'close'});
    ...
    switch (props.variant) {
        // the visual UI stuff for basic case
        case 'basic': {
            return (
        ...
        // THe visual UI stuff for full case
        case 'full': {
            return (
